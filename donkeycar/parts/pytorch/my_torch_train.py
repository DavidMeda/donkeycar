...
def train(cfg, tub_paths, model_output_path, model_type):
    ...
    training(epochs, model=model, loss_func=criterion, opt=optimizer, scheduler=scheduler,
             train_dl=train_data, val_dl=valid_data, model_name=model_name, output_path=output_path, cfg=cfg, alpha=alpha)
...
def loss_epoch(model, loss_func, dataset_dl, opt=None):
    ...
    for inputs, labels in dataset_dl:
        ...
        # print(predict)
        # obtain loss weighted
        loss_1 = loss_func(predict[0], labels[0])
        loss_2 = loss_func(predict[1], labels[1])
        loss = loss_1+loss_2
...
def save_model(epoch, model, opt, loss_func, scheduler, stat_train, stat_valid, name_model, output_path):
    ...
    stat_train.to_csv(os.path.join(output_path, name_model +
                      '_train_epoch-' + str(epoch) + '.csv'))
    stat_valid.to_csv(os.path.join(output_path, name_model +
                      '_valid_epoch-' + str(epoch) + '.csv'))

    print("Save model at {}".format(os.path.join(
        output_path, name_model+'_epoch-' + str(epoch) + '.pth')))


def validation(epoch, model, loss_func, val_dl, output_path, model_name, alpha):
    ...
    print("VALIDATION - Epoca {} - val loss: {:.6f} ({:.6f})".format(epoch,
          val_loss, np.array(loss_batch).mean()))
...
def training(epochs, model, loss_func, opt, scheduler, train_dl, val_dl, model_name, output_path, cfg, alpha):
        min_val_loss = np.Inf
        n_epochs_stop = 5
        early_stop = False
        epochs_no_improve = 0
        stat_train = pd.DataFrame(
            columns=['epoch',  'train loss', 'train loss batch'])
        stat_valid = pd.DataFrame(
            columns=['epoch',  'valid loss', 'valid loss batch'])
        ...
        for epoch in range(epochs):
            model.train()
            # print("sono qui")
            train_loss, train_loss_batch = loss_epoch(
                model, loss_func, train_dl, opt)
            # print("fine loss")
            print('TRAIN - Epoca[{}/{}] - train loss: {:.6f} ({:.6f})'.format(
                epoch+1, epochs, train_loss, np.array(train_loss_batch).mean()))
            stop_time = timeit.default_timer()
            print("Execution time: " + str(stop_time - start_time))
            stat_train = stat_train.append(
                {'epoch': epoch, 'train loss': train_loss, 'train loss batch': train_loss_batch}, ignore_index=True)

            # validation
            val_loss, valid_loss_batch = validation(
                epoch, model, loss_func, val_dl, output_path,  model_name, alpha)
            stat_valid = stat_valid.append(
                {'epoch': epoch, 'valid loss': val_loss, 'valid loss batch': valid_loss_batch}, ignore_index=True)

            # aggiusto learning rate
            scheduler.step(val_loss)

            if min_val_loss-val_loss >= float(cfg.MIN_DELTA):
        ...
        if early_stop == False:
            save_model(epoch, model, opt, loss_func, scheduler,
                       stat_train, stat_valid, model_name, output_path)
